...
import { DisclosureArray, hashDisclosure, parseDisclosure } from './selective-disclosure.js';
...
export const verifyDisclosures = (disclosures, sd_alg, sd) => {
    let disclosedClaims = {};
...
export const verifySdJwt = async (sdJwt, jwks) => {
    try {
        ...
        if (parts.length > 1) {
            ...
            const sd = (payloadObject)._sd;
            const sd_alg = (payloadObject)._sd_alg;
        ...
        throw new Error(`Can't verify the SD-JWT: ${err}`);
    }
}


export const verifySdJwtFile = async (sdJwtPath, jwksPath, outJwtPath, outDisclosedPath) => {
