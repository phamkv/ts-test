...
export const createDisclosures = (hashAlg, claimValues, target) => {
    let disclosures = [];
    let sdDigests = [];
    const names = Object.keys(claimValues);
    const values = Object.values(claimValues);
    const salts = names.map(v => crypto.randomBytes(SALT_BYTE_SIZE));
...
export const encodeDisclosure = (disclosureArray) => {
...
const ianaToCryptoAlg = (hashAlg) => hashAlg.replace('-', '').toLowerCase(); 

export const hashDisclosure = (alg, disclosure) => {
    return jose.base64url.encode(crypto.createHash(ianaToCryptoAlg(alg)).update(disclosure).digest());
}
export const parseDisclosure = (disclosure) => {
    const input = jose.base64url.decode(disclosure);
    const parsed = JSON.parse(Buffer.from(input).toString());
    if (parsed.length != 3) {
